...
org_freq <- merge_by_group(asian_org, latino_org) %>% 
...
ols.model <- ols_its(reagan_org) + ggtitle("OLS") + labs(y = "Organizatioanl founding") + ylim(c(-1,9))

logged.ols.model <- ols_its(reagan_org %>% mutate(Freq = log(Freq))) + ggtitle("OLS with logged DV") + labs(y = "Organizatioanl founding (logged)") + ylim(c(-1, 4))

poisson.model <- ps_its(reagan_org) + ggtitle("Poisson") + ylim(c(-1,9))

nb.model <- ps_its(reagan_org) + ggtitle("Negative binominal") + ylim(c(-1,9))

(ols.model + logged.ols.model) / (poisson.model + nb.model)

ggsave("/home/jae/analyzing-asian-american-latino-civic-infrastructure/outputs/pred_plots.png", height = 5, width = 10)
...
lm.out <- lm(Freq ~ intervention + Percentage + pop_percentage + category + Type + presidency + senate + house, data = reagan_org)

lm_log.out <- lm(log(Freq) ~ intervention + Percentage + pop_percentage + category + Type + presidency + senate + house, data = reagan_org)
  
poisson.out <- glm(Freq ~ intervention + Percentage + pop_percentage + category + Type + presidency + senate + house, data = reagan_org, family = "poisson")

nb.out <- glm.nb(Freq ~ intervention + Percentage + pop_percentage + category + Type + presidency + senate + house, data = reagan_org)

stargazer(lm.out, lm_log.out, poisson.out, 
          nb.out, type = "text")
...
m = Boot(lm(Freq ~ Percentage + pop_percentage + category, data = subset(reagan_org, Year <= 1970 + i)), R = 1000)
