...
lm.out <- lm(Freq ~ Year + intervention + Year*intervention + category, data = reagan_org)

lm_log.out <- lm(log(Freq) ~ Year + intervention + Year*intervention + category, data = reagan_org)
  
poisson.out <- glm(Freq ~ Year + intervention + Year*intervention + category, data = reagan_org, family = "poisson")

nb.out <- glm.nb(Freq ~ Year + intervention + Year*intervention + category, data = reagan_org)
...
# Model 
m = Boot(lm(log(Freq) ~ Percentage + pop_percentage + category + presidency + senate + house, data = subset(reagan_org, Year <= 1970 + i)), R = 1000)
