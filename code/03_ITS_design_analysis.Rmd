...
ols.model <- ols_its(reagan_org) + ggtitle("OLS") + labs(y = "Organizational founding") + xlim(c(1970,1990))

logged.ols.model <- ols_its(reagan_org %>% mutate(Freq = log(Freq))) + ggtitle("OLS + ln(DV)") + labs(y = "Organizational founding ") + xlim(c(1970,1990))
  
ggarrange(ols.model, logged.ols.model)
...
poisson.model <- ps_its(reagan_org) + ggtitle("Poisson") + xlim(c(1970,1990))

nb.model <- ps_its(reagan_org) + ggtitle("NB") + xlim(c(1970,1990))

ggarrange(poisson.model, nb.model)

ggsave("/home/jae/analyzing-asian-american-latino-civic-infrastructure/outputs/non_ols_plots.png")
