...
# Structual change approach 
opt_breaks <- breakpoints(breakpoints(log(Freq) ~ Year, data = reagan_org), breaks = 2)
...
reagan_org_fit <- reagan_org %>% filter(between(Year, 1960, 1988))
...
\hline \\[-1.8ex] 
 Year & 0.056$^{***}$ & 0.052$^{***}$ \\ 
  & (0.019) & (0.017) \\ 
  & & \\ 
 intervention1 & 73.122$^{*}$ & 13.393 \\ 
  & (38.333) & (94.475) \\ 
  & & \\ 
 TypeCBO & 0.596$^{***}$ & 0.536$^{***}$ \\ 
  & (0.098) & (0.142) \\ 
  & & \\ 
 categoryLatino & 0.447$^{***}$ & 0.222$^{*}$ \\ 
  & (0.090) & (0.127) \\ 
  & & \\ 
 Year:intervention1 & $-$0.037$^{*}$ & $-$0.007 \\ 
  & (0.019) & (0.048) \\ 
  & & \\ 
 Constant & $-$111.282$^{***}$ & $-$101.766$^{***}$ \\ 
  & (36.653) & (33.691) \\ 
  & & \\ 
\hline \\[-1.8ex] 
Observations & 220 & 95 \\ 
R$^{2}$ & 0.286 & 0.225 \\ 
Adjusted R$^{2}$ & 0.270 & 0.181 \\ 
Akaike Inf. Crit. & 450.693 & 181.052 \\ 
\hline 
\hline \\[-1.8ex] 
...
lm_log.out1 <- lm(log(Freq) ~ Year + intervention + Year*intervention + Type + category, data = reagan_org_fit)
...
add_df(lm_log.out)$df
add_df(lm_log.out1)$df
...
OLS AIC:389.473
Logged OLS AIC:450.693
Poisson AIC:352.022
Negative binomial AIC:353.298
...
fed_boot <- boot_merged %>%
  ...
  geom_vline(xintercept = c(1980), linetype = "dashed", size = 1, color = "red") +  
  annotate("rect", xmin = 1970, xmax = 1988, ymin = -0.5, ymax = 0, alpha = .2)
...
pop_boot <- boot_merged %>%
  ...
  labs(x = "Year", y = "OLS coefficients", 
       ...
       subtitle = "With bootstrapped CIs") +
  geom_vline(xintercept = c(1980), linetype = "dashed", size = 1, color = "red") +
  annotate("rect", xmin = 1970, xmax = 1988, ymin = -2, ymax = 0, alpha = .2)
