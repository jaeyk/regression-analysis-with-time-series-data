...
script_list <- list(make_here("/home/jae/analyzing-asian-american-latino-civic-infrastructure/functions/misc.r"),
     ...
     here("functions", "visualize_its.r"),
     here("functions", "span_loess.r"))
...
ie_dic <- dfm(corpus(ie_processed), 
              dictionary = dictionary(list(budget =                                       c("spending","grants","grant","funding","funds","fund"))))

# Add to the original data  
ie_processed$budget <- convert(ie_dic, to = "data.frame")[,2]

# Fill in 0 dates
all_years <- seq(as.Date("1976/1/1"), as.Date("1987/12/31"), "day")
...
# Fill in 0 years
all_years <- seq(min(ie_processed$year), max(ie_processed$year))

# Turn it into a dataframe 
all_years <- data.frame(year = all_years)
  
# Count by year and type
ie_n <- ie_n %>% right_join(all_years, by = "year")

# reagan_plot 

ie_plot <- ggplot(ie_n, aes(x = year)) +
    geom_col(aes(y = n), alpha = 0.3) +
    geom_point(aes(y = count), col = "black") +
    geom_line(data = (ie_n[!is.na(ie_n$count), ]),
              aes(y = count), col = "red") +
    labs(x = "Year", y = "Count",
...
# Count by year and type
reagan_n <- reagan_n %>% right_join(all_years, by = "year")

# reagan_plot 

reagan_plot <- ggplot(reagan_n, aes(x = year)) +
    geom_col(aes(y = n), alpha = 0.3) +
    geom_point(aes(y = count), col = "black") +
    geom_line(data = (reagan_n[!is.na(reagan_n$count), ]),
              aes(y = count), col = "red") +
    labs(x = "Year", y = "Count",
...
```{r}
agenda_pattern <- ggplot(agenda_reagan) +
  
   # geom_histogram(aes(x = year), stat = "count") + 
  #  geom_vline(xintercept = c(as.Date("Jan 20, 1981", format = "%b%d%Y")), col = "red", linetype ="dashed") +
    geom_point(aes(x = year, y = budget), size = 2, alpha = 0.3) +
    geom_smooth(aes(x = year, y = budget), span = 0.1) +
    labs(x = "Year" , y = "Count",  
         title = "The frequency of budget related terms in Reagan related articles")  

#ggsave(make_here("/home/jae/analyzing-asian-american-latino-civic-infrastructure/outputs/dic_newspaper.png"), width = 10)
```
...
nb.model <- ps_its(reagan_org) + ggtitle("Negative binomial") 
...
pred_org_type <- ols_its(reagan_org %>% mutate(Freq = log(Freq))) + 
...
ggsave("/home/jae/analyzing-asian-american-latino-civic-infrastructure/outputs/pred_org_type.png")
...
lm.out <- lm(Freq ~ Year + intervention + Year*intervention + Type + category, data = reagan_org)

lm_log.out <- lm(log(Freq) ~ Year + intervention + Year*intervention + Type + category, data = reagan_org)
  
poisson.out <- glm(Freq ~ Year + intervention + Year*intervention + Type + category, data = reagan_org, family = "poisson")

nb.out <- glm.nb(Freq ~ Year + intervention + Year*intervention + Type + category, data = reagan_org)

stargazer(lm.out, lm_log.out, poisson.out, nb.out, 
          title = "ITS design analysis results", 
    #      align = TRUE,
          dep.var.labels = c("Founding rate", "Founding rate (logged)",
...
fit_ols <- function(df){
  lm(Freq ~ Year + intervention + Year*intervention + category, data = df)
}

fit_ols_ln_dv <- function(df){
  lm(log(Freq) ~ Year + intervention + Year*intervention + category, data = df)
}

fit_poisson <- function(df){
  glm(Freq ~ Year + intervention + Year*intervention + category, data = df, family = "poisson")
}

fit_neg_binomial <- function(df){
  glm.nb(Freq ~ Year + intervention + Year*intervention + category, data = df)
}

# Grouped modeling

grouped_modeling <- function(df, fit_model){
df %>%
  group_by(Type) %>%
  nest() %>%
  ...
  filter(term %in% c("intervention1", "Year:intervention1"))}

grouped_output <- bind_rows(grouped_modeling(reagan_org, fit_ols) %>% mutate(model = "OLS"),
          grouped_modeling(reagan_org, fit_ols_ln_dv) %>% mutate(model = "OLS with logged DV"),
          grouped_modeling(reagan_org, fit_poisson) %>% mutate(model = "Poisson"),
          grouped_modeling(reagan_org, fit_neg_binomial) %>% mutate(model = "Negative binomial")) 
...
pred_org_group <- ols_its_only_cbo(reagan_org %>% mutate(Freq = log(Freq))) + 
...
ggsave("/home/jae/analyzing-asian-american-latino-civic-infrastructure/outputs/pred_org_group.png")

ggarrange(pred_org_type, pred_org_group, ncol = 1, nrow = 2)

ggsave("/home/jae/analyzing-asian-american-latino-civic-infrastructure/outputs/pred_subgroups.png", height = 10)
...
fed_boot <- boot_merged %>%
  ...
  geom_vline(xintercept = c(1980), linetype = "dashed", size = 1, color = "red") +  
  annotate("rect", xmin = 1964, xmax = 2010, ymin = -0.5, ymax = 0, alpha = .2)
