...
# Check whether there exists NAs
sum(is.na(asian_lat_longs$latitude)) # 10 missing addresses 
sum(is.na(latino_lat_longs$latitude)) # 26 missing addresses 

# Fix the problematic addresses using Google maps 

asian_NA_list <- asian_lat_longs$Address[which(is.na(asian_lat_longs$latitude))]

asian_NA_list[1]

url <- paste("https://www.google.com/search?q=asus")
...
url <- "https://www.google.com/search?q=%22101+8th+Street%2C+Suite+101+Oakland%2C+CA+94607%22&oq=%22101+8th+Street%2C+Suite+101+Oakland%2C+CA+94607%22&aqs=chrome..69i57j35i39.34360j0j4&sourceid=chrome&ie=UTF-8"

first_page <- read_html(url)

test1 <- first_page %>% html_nodes("div") %>% html_text()
first_page %>% html_nodes("span.desktop-title-subconten") 

new_address <- nodes[html_attr(nodes, "style") == "display:inline-block;float:right;padding-left:5px;padding-top:20px"] 

first_page %>% html_text()

<div style="min-width:100%"><div class="Uekwlc" style="display:inline-block;float:right;padding-left:5px;padding-top:20px"></div><div style="display:inline"><b><div><div style="padding-top:18px" class="vk_sh vk_bk"><div class="desktop-title-content">101 8th St #101</div> <span class="desktop-title-subcontent">Oakland, CA 94607</span></div></div></b> </div><div class="Uekwlc QzqPSc" style="display:block"><!--n--></div></div>

gsub("Open in Maps","", test[26])

first_page %>% html_nodes("div") %>% html_text()
```
