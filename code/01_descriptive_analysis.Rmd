...
pop_density_plot <- ggplot(org_count, aes(x = decade, y = ratio, 
    ...
    scale_fill_npg() +
    labs(x = "Decade", 
         y = "N of orgs founded (per 1 million population)", 
...
org_df %>% 
  ...
  count(decade, national, Type, category) %>%
  mutate(
      prop = n / sum(n),
      prop = round(prop, 2)
    ) %>%
...
```{r}
org_df %>% 
  filter(name_value == 1 & !is.na(decade)) %>% 
  stacked_bar_plot(decade, name_type, category) +
    facet_wrap(~category)

org_df %>%
  stacked_bar_plot(national, category) +
    facet_wrap(~category)
