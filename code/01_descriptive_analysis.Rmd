...
lat_longs_county <- read_csv(here("processed_data", "org_lat_logs_county.csv"))[,-1]

```

### Making maps

#### Dots (individual level)
...
mapshot(lf, file = here("outputs", "asian_org_map_interactive.png"),
...
# Join the two data 
df_county <- left_join(org_address, lat_longs_county)

# Transform 
df_county <- df_county %>%
    group_by(County, category, Type, latitude, longtitude) %>%
    count()

# Turn df_county into sf object
counties_sf <- sf::st_as_sf(df_county, 
                      coords = c("longtitude", 
                                 "latitude"),
                      crs = 4326, 
                      agr = "identity")

# Import US county shape files

us_counties <- tigris::counties()

# Set the CRS
us_counties <- st_transform(us_counties, 4326)

joined_sf <- st_join(us_counties, counties_sf)
